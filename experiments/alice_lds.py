...
def fit_hmm(Xs, Xtest, D_hmm, N_samples=100):
    ...
    def compute_pred_ll():
        ...
        for Xtr, Xte, model in zip(Xs, Xtest, models):
            pred_ll += model.log_likelihood(np.vstack((Xtr, Xte))) - model.log_likelihood()
        return pred_ll
...
if __name__ == "__main__":
    ...
    all_results = []
    Ds = np.array([10])
    # models = ["SBM-LDS", "HMM", "LNM-LDS", "SBM-LDS (pMCMC)"]
    # methods = [fit_lds_model, fit_hmm, fit_ln_lds_model, fit_lds_model_with_pmcmc]
    models = ["SBM-LDS", "HMM", "LNM-LDS"]
    methods = [fit_lds_model, fit_hmm, fit_ln_lds_model]
    # models = ["SBM-LDS", "HMM"]
    # methods = [fit_lds_model, fit_hmm]
